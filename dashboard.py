...
def get_weather(session):
    try:
        ...
        # Gust (Knots)
        gust_ms = current.get('wind_gust', wind_ms) # Default to speed if no gust
        ...
        forecast_items = []
        if hourly:
            # We want the next 3 hours (indices 1, 2, 3 usually, as 0 is "now")
            for i in range(1, 4): 
            ...
            # Get max rain probability for current summary
...
def get_transport(session):
    ...
    try:
        ...
        for event in events[:4]:
            ...
            # Ensure dep_dt is timezone aware
        ...
        for component in cal.walk():
            if component.name == "VEVENT":
                ...
                if type(dtstart) is datetime:
                    ...
                    time_str = dtstart.strftime("%I:%M%p").lower().lstrip("0")
                    date_str = dtstart.strftime("%a %d")
                else:
...
def main():
    ...
    while True:
        try:
            ...
            # --- FIX: Strict Dimensions & No Scaling ---
            # We set the window size to exactly 800x480.
            # IMPORTANT: We disable scrollbars entirely in CSS to prevent offset.
            hti = Html2Image(browser_executable=browser_path, 
                           custom_flags=['--no-sandbox', '--hide-scrollbars', '--force-device-scale-factor=1', '--window-size=800,480'])
            
            # We save directly to the output. Cropping shouldn't be needed if CSS is perfect.
            # But to be 100% safe against the "cut off" issue, we render slightly larger and crop.
            temp_file = "temp_render.png"
            hti.screenshot(html_str=html_out, css_file=CSS_FILE, save_as=temp_file, size=(800, 480))
            
            # Ensure it is exactly 800x480
